...
{ 
	DMARxDscrTab=ow_malloc(SRAMIN,ETH_RXBUFNB*sizeof(ETH_DMADESCTypeDef));//申请内存
	DMATxDscrTab=ow_malloc(SRAMIN,ETH_TXBUFNB*sizeof(ETH_DMADESCTypeDef));//申请内存  
	Rx_Buff=ow_malloc(SRAMIN,ETH_RX_BUF_SIZE*ETH_RXBUFNB);	//申请内存
	Tx_Buff=ow_malloc(SRAMIN,ETH_TX_BUF_SIZE*ETH_TXBUFNB);	//申请内存
...
{ 
	ow_free(SRAMIN,DMARxDscrTab);//释放内存
	ow_free(SRAMIN,DMATxDscrTab);//释放内存
	ow_free(SRAMIN,Rx_Buff);		//释放内存
	ow_free(SRAMIN,Tx_Buff);		//释放内存  
}
