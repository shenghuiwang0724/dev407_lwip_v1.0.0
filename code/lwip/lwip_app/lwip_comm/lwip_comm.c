...
u8 lwip_comm_mem_malloc(void)
{
	...
	memp_memory=ow_malloc(SRAMIN,mempsize);	//为memp_memory申请内存
	ramheapsize=LWIP_MEM_ALIGN_SIZE(MEM_SIZE)+2*LWIP_MEM_ALIGN_SIZE(4*3)+MEM_ALIGNMENT;//得到ram heap大小
	ram_heap=ow_malloc(SRAMIN,ramheapsize);	//为ram_heap申请内存 
...
{ 	
	ow_free(SRAMIN,memp_memory);
	ow_free(SRAMIN,ram_heap);
}
//lwip 默认IP设置
//lwipx:lwip控制结构体指针
